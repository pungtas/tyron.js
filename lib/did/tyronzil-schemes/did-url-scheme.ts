...
export default class DidUrlScheme extends DidScheme {
  ...
  private constructor(input: UrlInput) {
  ...
  public static async validate(did: string): Promise<DidUrlScheme> {
    ...
    if (!did.startsWith(PREFIX)) {
      throw new ErrorCode(
        "CodeIncorrectDidPrefix",
        "The given DID does not have the right prefix"
      );
    }
    ...
    if (
      ...
      throw new ErrorCode(
        "CodeIncorrectNetwork",
        "The network namespace is invalid"
      );
    }
